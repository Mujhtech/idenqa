...
**Status — updated 22 September 2026:** Implemented evaluation runtime; ML-01 and ML-04 remain In progress. **Selected:** ONNX Runtime, with presentation-attack detection (PAD) / liveness as the first capability and a first-party selfie-analysis route as the primary engineering direction. No trained model, biometric threshold or production liveness claim is accepted by this slice.

## 1. Implemented boundary

`cmd/model-runner` composes the private TLS gRPC ModelRunnerService. The owned adapter in `adapters/models/onnx` runs official ONNX Runtime through an isolated Python subprocess. Core uses the public model contract and contains no ONNX tensor types or native library linkage. The Go binary requires a separately installed Python environment plus the pinned graph files for the selected capability; detector-only selfie analysis needs only its detector graph.
...
The detector-only `idenqa.check.selfie_analysis` route accepts one selfie or a complete temporal selfie sequence. It emits separate, always-inconclusive `face_count`, `face_position`, `head_pose`, `selfie_image_quality`, and, for a temporal capability, `temporal_integrity` signals. The native workload returns only bounded reason codes; pixels, boxes, landmarks, crops, tensors and embeddings do not cross into Core. The current implementation uses the pinned YuNet detector plus explicitly versioned image/geometry heuristics for face selection, framing, pose, brightness, contrast, sharpness and glare. It also classifies exact duplicate temporal frames and invalid sequence duration. These are capture-quality and integrity diagnostics, not biometric assurance, liveness, fraud, identity, age, public-figure, account-comparison, glasses or face-covering determinations.
...
The worker binds one tenant, policy ID, immutable capture-profile digest, explicit evaluation and selfie requirement to a model configuration. Legacy bindings without `evaluation` retain PAD/document-match inference for compatibility; new routes select their exact manifest capability. Migration 38 adds tenant-scoped, RLS-protected `model_requests` and `model_dispatches`. Planning commits the exact request, every required evidence grant, attempt and task intent together. Dispatch checks current processing authority and the current running attempt. Concurrent or ambiguous dispatches cannot blindly repeat an evidence read; the first validated receipt remains immutable. [Composed runtime](composed-verification-runtime-v0.1.md) now permits a model bundle alongside one provider route for the same tenant/policy/profile. Synthetic processing remains mutually exclusive.
...
The candidate's [preprocessing](https://github.com/facenox/face-antispoof-onnx/blob/fa6489fb221dcf6b803095ab4b15a0fa0f56cfe5/src/inference/preprocess.py) and [limitations](https://github.com/facenox/face-antispoof-onnx/blob/fa6489fb221dcf6b803095ab4b15a0fa0f56cfe5/docs/LIMITATIONS.md) require contextual face preparation and restrict supported capture/attack conditions. The reference implementation now supplies a pinned YuNet detector and 1.5× contextual crop/resize pipeline, with synthetic/native verification. Web temporal-sequence transport, digest-chain provenance and complete sequence consumption by selfie-analysis diagnostics are implemented; production acceptance of detector/preprocessing quality, active-liveness semantics and temporal active-liveness prediction remains open. A correctly sized uploaded image does not prove that preprocessing occurred. Therefore even the candidate's native output stays inconclusive.
...
Prepare the closed JSON [runner settings](../internal/bootstrap/modelrunner/process.go) with absolute `python`, certificate, private-key and credential paths; an explicit listen address; and the private HTTPS evidence gateway plus its CA/credential files. PAD and matching also require an absolute `model_file`. Selfie analysis sets `selfie_analysis: true`, supplies the absolute `detector_file`, may leave `model_file` empty, and uses the detector digest as the model artefact digest because no unrelated scoring graph is loaded. `model` uses the [adapter configuration](../adapters/models/onnx/adapter.go): tenant ID, model registration ID/reference/digest, manifest, `width: 128`, `height: 128`, and mandatory `evaluation_only: true`. Selfie-analysis provenance pins `AnalysisPreprocessingDigest(face_preparation)` and `AnalysisOutputSchemaDigest(temporal)`; its capability advertises the exact ordered signal list from `SelfieAnalysisSignals(temporal)`, one selfie evidence type, no required assurance, and one grant for still-image analysis or two to 32 grants for a temporal sequence. Temporal PAD is rejected until a PAD predictor consumes the complete sequence. Version 1.0 request envelopes remain consumable under the same-major compatibility rule, but temporal sequences, transient-derived-data restrictions and typed quality use 1.1.
...
The selfie-analysis Go adapter tests cover separate bounded signals, typed unacceptable quality, temporal duplicate classification and the always-inconclusive boundary. Native geometry/classification tests require the locked OpenCV environment. They do not establish thresholds or representative accuracy.
