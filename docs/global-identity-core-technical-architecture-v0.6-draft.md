...
The independently versioned S3 distribution composes the public API evidence-ingress process and the background worker against the same bucket, prefix, region, and endpoint. The API receives owned immutable-object and key ports; the worker receives only its owned exact-deletion and lifecycle capability. AWS SDK types and dependencies remain outside the root core module, and an S3 worker must not be paired with the root-local API, or vice versa, for one installation.
...
The 18 September 2026 evolution decision selects the versioned public event catalogue, endpoint event-type subscriptions and scalable fanout. `contracts/webhook/v1` publishes the canonical envelope with one JSON schema and compatibility fixture per event type and version; evolution is additive within a major `schema_version`. `data` carries references plus safe summary fields (status, outcome, revisions, reason codes, timestamps, assurance lists, masks, counts) that are redacted at construction, so no raw evidence, biometric template, identity value, credential or provider payload is signed, stored or re-sent. Endpoints carry a validated `event_types` list of exact catalogue names or the single `*` entry (bounded to 64, defaulting to `["verification.completed"]`), where `*` includes newly added events except explicit-only ones. Each owning feature commits its transition, the reference-only event and a fanout task together; a fenced, resumable `webhook.fanout` task pages subscribed enabled endpoints in bounded batches and commits delivery intents per batch, replacing the fail-closed 1024-endpoint transaction guard. The selected catalogue is the section 30.1 list plus `verification.cancelled` and `decision.corrected`, excluding `provider.degraded` until provider health routing exists. Per-endpoint payload-version pinning remains unresolved.

Manual replay creates a distinct delivery linked by `replay_of`, preserving the exact original body and signed event ID. It requires an exhausted source and enabled endpoint, starts a fresh bounded delivery window and retains event-ID deduplication at the receiver. The CLI writes display-once material only to a newly created owner-only file and requires confirmation for rotate, disable and replay. Per-endpoint payload-version pinning, delivery/attempt-row retention and the provider-health `provider.degraded` event remain separate gaps.
...
**Selected — 22 September 2026:** document-type alternatives and their required
sides belong to the capture profile and immutable session pin. Core records the
chosen branch separately, with expected-version and idempotent mutation, and
uses it consistently for upload permission, capture completion and recovery.
Selection activates a pinned alternative; it does not rewrite requirements or
assert assurance. A subject cannot switch a requirement to another document
after an upload intent exists. Signed experience and host configuration cannot
authorise additional document types or remove sides from the selected branch.
Repository/package section 14.2 owns the detailed package contract.
...
**Selected — updated 22 September 2026:** ONNX Runtime is the initial inference runtime for locally hosted predictive models, behind the isolated model-runner boundary and the selected versioned gRPC transport. Idenqa-owned biometric models are the primary direction; external providers remain optional capability adapters for tenants that cannot run the models or need other services. The Go core remains independent of ONNX types, native libraries and binding language. Model and runtime artefacts must be packaged and pinned for OCI deployment. **Selected first capability:** PAD / liveness. The reference implementation uses official Python ONNX Runtime 1.29.0 with CPUExecutionProvider in an isolated subprocess, pinned native/model/configuration semantics, scoped evidence redemption and durable Core requests/receipts. The initial passive-PAD output is evaluation-only and always inconclusive. A pinned YuNet/contextual crop path and offline, aggregate-only evaluation runner exercise the same image pipeline; see [PAD evaluation](pad-evaluation-v0.1.md). An evaluation-only [face-matching reference](face-matching-runtime-v0.1.md) binds document and selfie grants to one attempt, performs whole-image portrait selection and a pinned five-landmark alignment, and compares embeddings inside the native workload. The detector-only selfie-analysis capability emits independent face-count, face-position, head-pose, image-quality and optional temporal-integrity signals for a still image or complete sequence; only bounded reason codes cross the native boundary. Every current model signal remains inconclusive. [Composed runtime](composed-verification-runtime-v0.1.md) joins optional provider routes with exact explicitly selected model capabilities under the same tenant/policy/profile and persists explicit dependency/fallback graphs. Overlapping public signals are allowed only within one connected fallback-equivalent chain whose routes declare the same non-empty correlation group and ordered signals, so only the selected branch can emit the fact; distinct correlated signals still retain their shared evidence/request lineage. Public evidence uploads can persist a complete ordered digest-chained Web challenge sequence, and temporal-capable model requests bind each frame grant. Model-contract v1.1 forbids persisted derived model data and non-zero derived retention, and restricts unacceptable quality to inconclusive. An accepted trained model, representative acceptance of preprocessing/alignment and the new quality/pose heuristics, active-liveness meaning, production occlusion/accessory classifiers, quality thresholds, hardened OCI image and hardware/resource acceptance remain TBD in the repository/package draft; repository mechanics do not establish production liveness.
