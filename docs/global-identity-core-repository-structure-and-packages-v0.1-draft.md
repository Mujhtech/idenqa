...
A session pins its policy ID at creation; its active revision is selected when authoring the immutable decision snapshot. Therefore activation may affect sessions without a snapshot, but cannot change an already pinned snapshot or decision. Public simulation, diff and regression tooling, production policy-to-check planning and future operator/Console approval workflows remain separate gaps.
...
The initial passive-PAD adapter is **evaluation-only**: every successful inference is inconclusive and cannot establish liveness. A candidate artefact has native smoke-test evidence, but a production trained model remains **TBD**. Pinned YuNet detection, contextual cropping and an offline evaluation command are now implemented in the reference adapter/composition; [PAD evaluation v0.1](pad-evaluation-v0.1.md) owns the operational details and public dataset shortlist. Production acceptance of face localisation/preprocessing, temporal/challenge capture, thresholds, model/dataset rights, representative quality evaluation, hardened OCI packaging, kernel resource/egress limits and supported hardware remain unresolved. The Python CPU binding is the implemented reference, not a decision to require Python in Core or exclude future bindings/accelerators. Runtime availability does not establish PAD, face-match or document-authenticity performance.
...
**Face-matching engineering increment — 7 September 2026:** The user authorised face matching after evaluation tooling. `adapters/models/onnx` now owns an evaluation-only document/selfie box-extraction and embedding/cosine path; `internal/model` owns its explicit two-requirement route and its PostgreSQL adapter creates both scoped grants atomically. Embeddings never cross the native workload boundary into Core, and the public signal remains inconclusive. Existing model contracts and dependencies are reused. [Face-matching runtime](face-matching-runtime-v0.1.md) defines input roles, preprocessing/output pins, bounds, synthetic acceptance evidence and unresolved model/alignment/pair-evaluation requirements. This implementation does not select a trained face-recognition model or threshold.

**Composed-runtime increment — 7 September 2026:** The user authorised joining document, PAD and matching signals into one verification workflow. Bootstrap now composes one provider with a bounded model bundle for the same tenant/policy/profile, rejects duplicate checks/output keys and dispatches exact saved configurations. The existing processing transaction owns all request/grant/check/task preparation atomically. API model-evidence dispatch requires distinct per-model gateway credentials and still validates each persisted request. [Composed runtime](composed-verification-runtime-v0.1.md) records configuration, acceptance evidence and remaining dynamic-routing/overlap-resolution boundaries. Public contracts, database schema and dependency selection remain unchanged.
...
- PostgreSQL owns `proposals` and `accepted_commands` with forced RLS, tenant predicates, append-only proposal state transitions (`expected_version` + monotonic `proposal_version`), stable `proposal_id` as event/cmd identity, and common append-only audit-chain linkage `proposal -> validation -> approval -> command -> effect -> result`. Migration, sqlc, and `platform/task` integration are TBD by the implementing brick; `platform/task` remains the only queue adapter.
...
**Proposed operations and products (later increments, not selected here):**

- Review copilot (evidence-linked, non-authoritative summaries), adaptive-route proposals, natural-language policy drafts/diffs, adversarial policy scenarios, accessibility/exception-path proposals, unfamiliar-document layout proposals, prompt/model registries and lifecycles, cost/rate metrics, evaluation/monitoring/audit records, and impact assessments remain Proposed and sequenced after the guardrail foundation (see build plan M6). Tenant-local fraud hypotheses (`internal/fraud` proposals) are the existing bounded non-AI proposal pattern and do not use `ProposalModel`.
...
18. **Selected — 6 September 2026:** initial-core policy activation and rollback use one authenticated tenant API key with dedicated `policies:activate` permission, expected activation version and common audit. A separate approver or step-up is not required for this initial-core surface. Read and immutable revision authorship use separate `policies:read` and `policies:write` permissions. Rollback explicitly selects a previously activated revision and appends a new activation; it never rewrites history. The v1 tenant-to-policy assignment, direct namespaced signal-to-fact mapping, one-to-one normalised outcomes, authoritative-only `unavailable`/`prohibited` derivation, and capture-complete plus required-checks-terminal `policy.author` trigger are **Selected**. P-01 implements public administration and signed cursors. Public simulation, diff and regression commands remain TBD; future operator approval and Console workflows are separate from the selected initial-core API-key boundary.

19. Lifecycle runtime integration details: production authoritative check-plan selection; policy-approved resumption to capture versus review; automatic expiry of any future separately persisted `created` phase; external-provider late-result reconciliation beyond the implemented stopped-session effect guards; resumption public permissions and token refresh contracts. Scalable completion fanout is resolved by item 20. L-03 resolves tenant/subject cancellation permissions, replay and current-session expiry as recorded above. These belong to the owning integration bricks before activation, not to the low-level L-01 persistence primitive. Public state compatibility is resolved by the selected 6 September alpha expansion above. Creation replay preserves its original result under the existing idempotency convention. Headgate effect transactions must establish serializable isolation before effect-claim queries when composing lifecycle writes.

20. **Selected — 18 September 2026:** webhook evolution beyond H-01 selects the versioned public event catalogue in `contracts/webhook/v1`, endpoint event-type subscriptions with exact names or `*`, outbox-first emission from every owning feature, and a fenced resumable `webhook.fanout` task, recorded in section 6.13. Per-endpoint payload-version pinning and delivery/attempt-row retention remain unresolved. H-01 resolves public endpoint administration, bounded delivery inspection, display-once secret receipts and exhausted-delivery replay.

21. Provider runtime beyond PR-01/PR-02: official-account sandbox evidence, external-success reconciliation and durable evidence delivery recovery, tenant route administration, dynamic credential rotation, provider-side deletion, budgets and fleet concurrency, production recipient/regional approval, callback intake, and workflow-metadata retention/purge for immutable request and dispatch receipts. The initial sandbox route and its at-most-one initial dispatch receipt do not resolve those operational gates.

22. ONNX/PAD acceptance: production licensed model and dataset rights; production acceptance of the implemented face localisation/contextual transform and its preprocessing provenance; approved evaluation data acquisition; trained face-matching model, document portrait/alignment acceptance and identity-separated pair evaluation; dynamic route administration, overlap resolution and dependency-ordered/fallback composition beyond the implemented same-profile parallel checks; temporal/challenge capture where active liveness is required; production threshold calibration and representative evaluation; hardened OCI packaging, kernel resource/egress limits and supported hardware. ONNX Runtime and PAD as first capability are **Selected** under section 6.9. The Python 1.29.0 CPU reference, bounded subprocess, durable workflow, contextual preparation and offline evaluation are implemented; model quality and production deployment gates remain open.

23. Review acceptance and external integrations: the selected review policy settings, independent escalation/correction rules, versioned operator revocation, controlled display and public contracts are implemented in section 6.10. External certification issuers, advanced queue assignment automation, tenant notification product surfaces and composed reviewer/subject acceptance remain unresolved. SSO remains deferred.
...
25. Identity extensions: production provider-specific structured extractors and automatic ingestion, country-specific normalisation beyond the three selected versioned transforms, and incremental deletion planning above the selected atomic limits remain **TBD**. The persistent subject model, source-trust distinction, identity receipt extension and linked-evidence deletion scope are selected in section 6.3.

26. Assurance follow-ups: production profiles and calibrated biometric operating points; jurisdiction/framework mapping records and approval; richer capability versions; field-level runner manifests; larger graph planning; and recovery workflows for assurance that expires between evaluation and commit. The version-one profile, pinning, source-time, independence, public API and canonical-context mechanisms above are implemented and do not resolve these production choices.
